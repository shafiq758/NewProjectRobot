...
class SboxApi(object):
    ...
    def download_splunk_tgz_version(self, version):
        try:
            ...
            r = requests.get(url)
            file_path = os.path.join("Upgrade2.2.3/Upgrade/Keywords", version)
    ...
    def download_upgrade_pack(self):
        try:
            url="https://downloads.geminidata.com/appliance/pack/2/3/0/gemini-appliance-2.2-to-2.3.pack.v7"
            r = requests.get(url)
            file_path = os.path.join("Upgrade2.2.3/Upgrade/Keywords/2.2.3to2.3", "gemini-appliance-2.2-to-2.3.pack.v7")
            with open(file_path, 'wb') as f:  
                f.write(r.content)
            return os.path.basename(file_path)
        except requests.exceptions.HTTPError as errh:
